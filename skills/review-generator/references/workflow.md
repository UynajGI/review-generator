...
**前置条件**：`pdf2doi`（`pipx install pdf2doi`）。

**1a. 提取 DOI / arXiv ID**：`pdf2doi refs/` 批量提取标识符。失败记录到 `refs/not_found.txt`。

**1b. 获取正式元数据**：对每个 DOI 用 Crossref API 查正式作者/年份/标题。arXiv ID 用 arXiv API。不再用 subagent 读 PDF 首页。

**1c. 排序重命名**：按年份升序 + 作者字母序，编号为 `NN_Author_YYYY_Keyword__DOI.pdf`（DOI 中 `/` 替换为 `_`）。编号即脉络——`01` 是最早的奠基工作。

**门禁**：所有 PDF 未达到 `NN_Author_YYYY_Keyword__DOI.pdf` 规范命名前，不得进入 Stage 2。`status.sh` 会统计规范命名比例并卡在 Stage 1。
